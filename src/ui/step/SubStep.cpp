...
std::unique_ptr<SubStep> SubStepTitle::execute(SubContext &context, const Reader &console_manager) {
...
std::unique_ptr<SubStep> SubStepPriority::execute(SubContext &context, const Reader &console_manager) {
    Task::Priority priority(console_manager.ReadPriority());
    context.set_task(Task::Arguments::Create(context.task_title(), priority));
    return SubFactory::GetNextSubStep(*this);
}

std::unique_ptr<SubStep> SubStepTime::execute(SubContext &context, const Reader &console_manager) {
